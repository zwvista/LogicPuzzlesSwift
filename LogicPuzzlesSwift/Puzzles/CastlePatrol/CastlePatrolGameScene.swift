...
class CastlePatrolGameScene: GameScene<CastlePatrolGameState> {
    var gridNode: CastlePatrolGridNode {
        get { getGridNode() as! CastlePatrolGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: CastlePatrolGameState, skView: SKView) {
        let game = game as! CastlePatrolGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: CastlePatrolGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: CastlePatrolGameState, to stateTo: CastlePatrolGameState) {
