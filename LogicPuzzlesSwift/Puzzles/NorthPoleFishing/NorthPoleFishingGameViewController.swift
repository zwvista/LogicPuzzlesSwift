...
class NorthPoleFishingGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: NorthPoleFishingGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: NorthPoleFishingGameState) {
        let game = game as! NorthPoleFishingGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: NorthPoleFishingGameState, to stateTo: NorthPoleFishingGameState) {
        let game = game as! NorthPoleFishingGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
