...
class NorthPoleFishingGameViewController: GameGameViewController {
    ...
    override func viewDidLoad() {
        ...
        scene = NorthPoleFishingGameScene(size: skView.bounds.size)
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! NorthPoleFishingGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! NorthPoleFishingGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! NorthPoleFishingGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! NorthPoleFishingGameState, to: stateTo as! NorthPoleFishingGameState)
