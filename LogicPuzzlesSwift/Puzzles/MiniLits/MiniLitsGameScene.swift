...
class MiniLitsGameScene: GameScene<MiniLitsGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: MiniLitsGameState, skView: SKView) {
        ...
        addGrid(gridNode: MiniLitsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
