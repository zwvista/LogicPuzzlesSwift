...
class LightBattleShipsGameScene: GameScene<LightBattleShipsGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: LightBattleShipsGameState, skView: SKView) {
        ...
        addGrid(gridNode: LightBattleShipsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
