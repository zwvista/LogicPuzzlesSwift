//
//  LightBattleShipsGameScene.swift
//  LogicPuzzlesSwift
//
...
class LightBattleShipsGameScene: GameScene<LightBattleShipsGameState> {
    var gridNode: LightBattleShipsGridNode {
        get {return getGridNode() as! LightBattleShipsGridNode}
        set {setGridNode(gridNode: newValue)}
    }
    
    func addBattleShip(color: SKColor, point: CGPoint, obj: LightBattleShipsObject, nodeName: String) {
    ...
    override func levelInitialized(_ game: AnyObject, state: LightBattleShipsGameState, skView: SKView) {
        let game = game as! LightBattleShipsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols + 1)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: LightBattleShipsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols + 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows + 1) / 2 + offset))
        ...
        // add LightBattleShips and marker
