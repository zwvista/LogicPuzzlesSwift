...
class NumberLinkGameViewController: GameGameViewController2<NumberLinkGameState, NumberLinkGame, NumberLinkDocument, NumberLinkGameScene> {
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! NumberLinkGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! NumberLinkGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! NumberLinkGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! NumberLinkGameState, to: stateTo as! NumberLinkGameState)
