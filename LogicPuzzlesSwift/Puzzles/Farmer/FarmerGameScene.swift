...
class FarmerGameScene: GameScene<FarmerGameState> {
    var gridNode: FarmerGridNode {
        get { getGridNode() as! FarmerGridNode }
        set { setGridNode(gridNode: newValue) }
    }
    
    func addObject(o: FarmerObject, s: AllowedObjectState, point: CGPoint, nodeName: String) {
    ...
    override func levelInitialized(_ game: AnyObject, state: FarmerGameState, skView: SKView) {
        let game = game as! FarmerGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: FarmerGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: FarmerGameState, to stateTo: FarmerGameState) {
