...
class TennerGridGameViewController: GameGameViewController {
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! TennerGridGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! TennerGridGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! TennerGridGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! TennerGridGameState, to: stateTo as! TennerGridGameState)
