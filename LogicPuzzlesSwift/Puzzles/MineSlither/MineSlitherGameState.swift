...
class MineSlitherGameState: GridGameState<MineSlitherGameMove> {
    var game: MineSlitherGame {
        get { getGame() as! MineSlitherGame }
        set { setGame(game: newValue) }
    }
    override var gameDocument: GameDocumentBase { MineSlitherDocument.sharedInstance }
    var objArray = [MineSlitherObject]()
    var pos2state = [Position: HintState]()
    
    override func copy() -> MineSlitherGameState {
        let v = MineSlitherGameState(game: game, isCopy: true)
        return setup(v: v)
    }
    func setup(v: MineSlitherGameState) -> MineSlitherGameState {
        _ = super.setup(v: v)
        v.objArray = objArray
        return v
    }
    
    required init(game: MineSlitherGame, isCopy: Bool = false) {
        super.init(game: game)
        guard !isCopy else {return}
        objArray = Array<MineSlitherObject>(repeating: .empty, count: rows * cols)
        updateIsSolved()
    }
    
    subscript(p: Position) -> MineSlitherObject {
        get { self[p.row, p.col] }
        set { self[p.row, p.col] = newValue }
    }
    subscript(row: Int, col: Int) -> MineSlitherObject {
        get { objArray[row * cols + col] }
        set { objArray[row * cols + col] = newValue }
    }
    
    override func setObject(move: inout MineSlitherGameMove) -> GameOperationType {
        let p = move.p
    ...
    override func switchObject(move: inout MineSlitherGameMove) -> GameOperationType {
        ...
        func f(o: MineSlitherObject) -> MineSlitherObject {
            switch o {
            case .empty:
