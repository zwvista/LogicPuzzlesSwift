...
class HolidayIslandGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: HolidayIslandGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: HolidayIslandGameState) {
        let game = game as! HolidayIslandGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: HolidayIslandGameState, to stateTo: HolidayIslandGameState) {
        let game = game as! HolidayIslandGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
