...
class KakuroGameViewController: GameGameViewController, GameDelegate {
    ...
    var scene: KakuroGameScene {
        get {return getScene() as! KakuroGameScene}
    ...
    func moveAdded(_ game: AnyObject, move: KakuroGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: KakuroGameState) {
        let game = game as! KakuroGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: KakuroGameState, to stateTo: KakuroGameState) {
        let game = game as! KakuroGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
