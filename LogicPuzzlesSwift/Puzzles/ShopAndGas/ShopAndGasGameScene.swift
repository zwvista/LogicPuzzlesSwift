...
class ShopAndGasGameScene: GameScene<ShopAndGasGameState> {
    var gridNode: ShopAndGasGridNode {
        get { getGridNode() as! ShopAndGasGridNode }
        set { setGridNode(gridNode: newValue) }
    }
    
    override func levelInitialized(_ game: AnyObject, state: ShopAndGasGameState, skView: SKView) {
        let game = game as! ShopAndGasGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: ShopAndGasGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
