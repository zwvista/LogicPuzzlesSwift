...
class FenceLitsGameScene: GameScene<FenceLitsGameState> {
    var gridNode: FenceLitsGridNode {
        get {return getGridNode() as! FenceLitsGridNode}
    ...
    override func levelInitialized(_ game: AnyObject, state: FenceLitsGameState, skView: SKView) {
        let game = game as! FenceLitsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols - 1)
        
        // addGrid
        let offset:CGFloat = 0.5
        addGrid(gridNode: FenceLitsGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols - 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows - 1) / 2 + offset))
