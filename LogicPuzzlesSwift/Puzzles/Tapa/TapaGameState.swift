...
class TapaGameState: GridGameState<TapaGameMove> {
    ...
    override func setObject(move: inout TapaGameMove) -> GameOperationType {
        let p = move.p
    ...
    override func switchObject(move: inout TapaGameMove) -> GameOperationType {
        let p = move.p
    ...
    */
    private func updateIsSolved() {
        ...
        for r in 0..<rows - 1 {
            for c in 0..<cols - 1 {
                let p = Position(r, c)
                if TapaGame.offset2.allSatisfy({os in
                    let o = self[p + os]
