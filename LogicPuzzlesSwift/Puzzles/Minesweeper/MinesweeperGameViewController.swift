...
class MinesweeperGameViewController: GameViewController, GameDelegate, MinesweeperMixin {
    ...
    @IBAction func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        var move = MinesweeperGameMove(p: p, obj: .empty)
    ...
    func startGame() {
        ...
        game = MinesweeperGame(layout: layout, delegate: self)
    ...
    func moveAdded(_ game: AnyObject, move: MinesweeperGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func updateMovesUI(_ game: MinesweeperGame) {
    ...
    func levelInitilized(_ game: AnyObject, state: MinesweeperGameState) {
        let game = game as! MinesweeperGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: MinesweeperGameState, to stateTo: MinesweeperGameState) {
        let game = game as! MinesweeperGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
