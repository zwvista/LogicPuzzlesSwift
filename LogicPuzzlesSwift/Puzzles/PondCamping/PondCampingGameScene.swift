...
class PondCampingGameScene: GameScene<PondCampingGameState> {
    var gridNode: PondCampingGridNode {
        get { getGridNode() as! PondCampingGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: PondCampingGameState, skView: SKView) {
        let game = game as! PondCampingGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: PondCampingGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
