...
class SukrokuroGameScene: GameScene<SukrokuroGameState> {
    var gridNode: SukrokuroGridNode {
        get { getGridNode() as! SukrokuroGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: SukrokuroGameState, skView: SKView) {
        let game = game as! SukrokuroGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: SukrokuroGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
...
//                    addDotMarker2(color: s == .normal ? .white : s == .complete ? .green : .red, point: point, nodeName: nodeName)
    ...
    override func levelUpdated(from stateFrom: SukrokuroGameState, to stateTo: SukrokuroGameState) {
