...
class YouTurnMeOnGameScene: GameScene<YouTurnMeOnGameState> {
    var gridNode: YouTurnMeOnGridNode {
        get { getGridNode() as! YouTurnMeOnGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: YouTurnMeOnGameState, skView: SKView) {
        ...
        addGrid(gridNode: YouTurnMeOnGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
