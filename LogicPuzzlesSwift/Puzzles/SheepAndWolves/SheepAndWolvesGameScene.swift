...
class SheepAndWolvesGameScene: GameScene<SheepAndWolvesGameState> {
    var gridNode: SheepAndWolvesGridNode {
        get { getGridNode() as! SheepAndWolvesGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: SheepAndWolvesGameState, skView: SKView) {
        let game = game as! SheepAndWolvesGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols - 1)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: SheepAndWolvesGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols - 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows - 1) / 2 + offset))
