...
class CrossroadsXGame: GridGame<CrossroadsXGameState> {
    ...
    init(layout: [String], delegate: CrossroadsXGameViewController? = nil) {
        ...
        for r in 0..<rows + 1 {
            ...
            for c in 0..<cols + 1 {
                ...
                let ch2 = str[2 * c + 1]
                self[r, c] = ch2 == " " ? CrossroadsXGame.PUZ_EMPTY : ch2.toInt!
            }
        }
        let g = Graph()
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + CrossroadsXGame.offset2[i]][CrossroadsXGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + CrossroadsXGame.offset[i]]!)
                    }
                }
            }
        }
