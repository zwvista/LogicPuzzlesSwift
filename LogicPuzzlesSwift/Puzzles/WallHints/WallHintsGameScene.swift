...
class WallHintsGameScene: GameScene<WallHintsGameState> {
    var gridNode: WallHintsGridNode {
        get { getGridNode() as! WallHintsGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: WallHintsGameState, skView: SKView) {
        let game = game as! WallHintsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols - 1)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: WallHintsGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols - 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows - 1) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: WallHintsGameState, to stateTo: WallHintsGameState) {
