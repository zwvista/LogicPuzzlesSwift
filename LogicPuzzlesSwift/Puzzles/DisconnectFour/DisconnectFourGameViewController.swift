//
//  DisconnectFourGameViewController.swift
//  LogicPuzzlesSwift
//
...
class DisconnectFourGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: DisconnectFourGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: DisconnectFourGameState) {
        let game = game as! DisconnectFourGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: DisconnectFourGameState, to stateTo: DisconnectFourGameState) {
        let game = game as! DisconnectFourGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
