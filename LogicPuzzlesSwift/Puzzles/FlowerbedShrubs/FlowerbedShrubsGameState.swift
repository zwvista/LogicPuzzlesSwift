//
//  FlowerbedShrubsGameState.swift
//  LogicPuzzlesSwift
//
...
class FlowerbedShrubsGameState: GridGameState<FlowerbedShrubsGameMove> {
    var game: FlowerbedShrubsGame {
        get { getGame() as! FlowerbedShrubsGame }
    ...
    func setup(v: FlowerbedShrubsGameState) -> FlowerbedShrubsGameState {
        _ = super.setup(v: v)
        v.objArray = objArray
        return v
    }
    
    required init(game: FlowerbedShrubsGame, isCopy: Bool = false) {
    ...
    override func setObject(move: inout FlowerbedShrubsGameMove) -> GameOperationType {
        var changed = false
        ...
        let p = move.p, p2 = p + FlowerbedShrubsGame.offset[dir]
    ...
    override func switchObject(move: inout FlowerbedShrubsGameMove) -> GameOperationType {
    ...
    private func updateIsSolved() {
        ...
        for r in 0..<rows - 1 {
            for c in 0..<cols - 1 {
                let p = Position(r, c)
                for i in 0..<4 {
                    guard self[p + FlowerbedShrubsGame.offset2[i]][FlowerbedShrubsGame.dirs[i]] != .line else {continue}
                    g.addEdge(pos2node[p]!, neighbor: pos2node[p + FlowerbedShrubsGame.offset[i]]!)
                }
            }
        }
