...
class SuspendedGravityGame: GridGame<SuspendedGravityGameState> {
    ...
    init(layout: [String], delegate: SuspendedGravityGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + SuspendedGravityGame.offset2[i]][SuspendedGravityGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + SuspendedGravityGame.offset[i]]!)
                    }
                }
            }
        }
        ...
        let state = SuspendedGravityGameState(game: self)
