//
//  PairakabeGameViewController.swift
//  LogicPuzzlesSwift
//
...
class PairakabeGameViewController: GameGameViewController2<PairakabeGameState, PairakabeGame, PairakabeDocument, PairakabeGameScene> {
    ...
    override func viewDidLoad() {
        ...
        scene = PairakabeGameScene(size: skView.bounds.size)
    ...
    override func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        var move = PairakabeGameMove(p: p, obj: .empty)
        if game.switchObject(move: &move) { soundManager.playSoundTap() }
    }
    
    override func newGame(level: GameLevel) -> PairakabeGame {
        PairakabeGame(layout: level.layout, delegate: self)
    }
}
