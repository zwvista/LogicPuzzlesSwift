...
class HiddenStarsGameScene: GameScene<HiddenStarsGameState> {
    var gridNode: HiddenStarsGridNode {
        get { getGridNode() as! HiddenStarsGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: HiddenStarsGameState, skView: SKView) {
        let game = game as! HiddenStarsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols + 1)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: HiddenStarsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols + 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows + 1) / 2 + offset))
