...
class BoxItAgainGameScene: GameScene<BoxItAgainGameState> {
    var gridNode: BoxItAgainGridNode {
        get {return getGridNode() as! BoxItAgainGridNode}
    ...
    override func levelInitialized(_ game: AnyObject, state: BoxItAgainGameState, skView: SKView) {
        ...
        addGrid(gridNode: BoxItAgainGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols - 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows - 1) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: BoxItAgainGameState, to stateTo: BoxItAgainGameState) {
