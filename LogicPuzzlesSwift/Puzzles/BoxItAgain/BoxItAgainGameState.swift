...
class BoxItAgainGameState: GridGameState, BoxItAgainMixin {
    ...
    override func copy() -> BoxItAgainGameState {
        let v = BoxItAgainGameState(game: game)
        return setup(v: v)
    }
    ...
    required init(game: BoxItAgainGame) {
            ...
            for c in 0..<cols - 1 {
                ...
                for i in 0..<4 {
                    if self[p + BoxItAgainGame.offset2[i]][BoxItAgainGame.dirs[i]] != .line {
                        g.addEdge(source: pos2node[p]!, neighbor: pos2node[p + BoxItAgainGame.offset[i]]!)
                    }
                }
            }
        }
