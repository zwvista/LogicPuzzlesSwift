    ...
    override func levelUpdated(from stateFrom: LoopyGameState, to stateTo: LoopyGameState) {
        ...
        for row in 0..<stateFrom.rows {
            for col in 0..<stateFrom.cols {
                ...
                func addHorzLine(objType: LoopyObject) {
                ...
                func removeHorzLine(objType: LoopyObject) {
                    if objType != .empty { removeNode(withName: horzLineNodeName) }
                }
                func addVertLine(objType: LoopyObject) {
                    guard objType != .empty else {return}
                    let pathToDraw = CGMutablePath()
                    let lineNode = SKShapeNode(path:pathToDraw)
                    switch objType {
                ...
                func removeVertLine(objType: LoopyObject) {
