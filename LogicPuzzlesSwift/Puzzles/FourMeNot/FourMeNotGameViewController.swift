...
class FourMeNotGameViewController: GameGameViewController, GameDelegate {
    typealias GM = FourMeNotGameMove
    ...
    override func startGame() {
        ...
        game = FourMeNotGame(layout: level.layout, delegate: self)
    ...
    func moveAdded(_ game: AnyObject, move: FourMeNotGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: FourMeNotGameState) {
        let game = game as! FourMeNotGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: FourMeNotGameState, to stateTo: FourMeNotGameState) {
        let game = game as! FourMeNotGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
