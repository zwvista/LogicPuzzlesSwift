...
class PouringWaterGameScene: GameScene<PouringWaterGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: PouringWaterGameState, skView: SKView) {
        ...
        addGrid(gridNode: PouringWaterGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
