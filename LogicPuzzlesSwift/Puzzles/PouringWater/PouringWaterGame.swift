...
class PouringWaterGame: GridGame<PouringWaterGameState> {
    ...
    init(layout: [String], delegate: PouringWaterGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + PouringWaterGame.offset2[i]][PouringWaterGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + PouringWaterGame.offset[i]]!)
                    }
                }
            }
        }
        ...
        let state = PouringWaterGameState(game: self)
