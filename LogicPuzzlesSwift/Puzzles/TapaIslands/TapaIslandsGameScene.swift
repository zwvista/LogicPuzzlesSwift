...
class TapaIslandsGameScene: GameScene<TapaIslandsGameState> {
    var gridNode: TapaIslandsGridNode {
        get {return getGridNode() as! TapaIslandsGridNode}
    ...
    override func levelInitialized(_ game: AnyObject, state: TapaIslandsGameState, skView: SKView) {
        ...
        addGrid(gridNode: TapaIslandsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: TapaIslandsGameState, to stateTo: TapaIslandsGameState) {
