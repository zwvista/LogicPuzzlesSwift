...
class NeighboursGameScene: GameScene<NeighboursGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: NeighboursGameState, skView: SKView) {
        ...
        addGrid(gridNode: NeighboursGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols - 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows - 1) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: NeighboursGameState, to stateTo: NeighboursGameState) {
