...
class DigitWormsGameScene: GameScene<DigitWormsGameState> {
    var gridNode: DigitWormsGridNode {
        get { getGridNode() as! DigitWormsGridNode }
        set { setGridNode(gridNode: newValue) }
    }

    override func levelInitialized(_ game: AnyObject, state: DigitWormsGameState, skView: SKView) {
        let game = game as! DigitWormsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: DigitWormsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
        ...
        for r in 0..<game.rows {
            for c in 0..<game.cols {
                ...
                let point = gridNode.centerPoint(p: p)
                let n = game[p]
                guard n != DigitWormsGame.PUZ_EMPTY else {continue}
    ...
    override func levelUpdated(from stateFrom: DigitWormsGameState, to stateTo: DigitWormsGameState) {
        for r in 0..<stateFrom.rows {
            for c in 0..<stateFrom.cols {
                ...
                if n1 != DigitWormsGame.PUZ_EMPTY {
