...
class GardenTunnelsGameScene: GameScene<GardenTunnelsGameState> {
    var gridNode: GardenTunnelsGridNode {
        get { getGridNode() as! GardenTunnelsGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: GardenTunnelsGameState, skView: SKView) {
        let game = game as! GardenTunnelsGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: GardenTunnelsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: GardenTunnelsGameState, to stateTo: GardenTunnelsGameState) {
