...
class TurnMeUpGameScene: GameScene<TurnMeUpGameState> {
    var gridNode: TurnMeUpGridNode {
        get { getGridNode() as! TurnMeUpGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: TurnMeUpGameState, skView: SKView) {
        ...
        addGrid(gridNode: TurnMeUpGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: TurnMeUpGameState, to stateTo: TurnMeUpGameState) {
