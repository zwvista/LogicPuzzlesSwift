...
class KakurasuGameViewController: GameGameViewController {
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! KakurasuGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! KakurasuGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! KakurasuGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! KakurasuGameState, to: stateTo as! KakurasuGameState)
