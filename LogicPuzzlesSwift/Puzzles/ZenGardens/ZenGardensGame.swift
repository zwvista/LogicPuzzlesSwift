...
class ZenGardensGame: GridGame<ZenGardensGameState> {
    ...
    init(layout: [String], delegate: ZenGardensGameViewController? = nil) {
        super.init(delegate: delegate)
        
        size = Position(layout.count / 2, layout[0].length / 2)
        dots = GridDots(rows: rows + 1, cols: cols + 1)
        objArray = Array<ZenGardensObject>(repeating: ZenGardensObject(), count: rows * cols)
        ...
        let state = ZenGardensGameState(game: self)
