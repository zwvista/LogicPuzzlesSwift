...
class BoxItAroundGameState: GridGameState<BoxItAroundGameMove> {
    ...
    override func setObject(move: inout BoxItAroundGameMove) -> GameOperationType {
        var changed = false
    ...
    override func switchObject(move: inout BoxItAroundGameMove) -> GameOperationType {
    ...
    private func updateIsSolved() {
        ...
        for r in 0..<rows - 1 {
            for c in 0..<cols - 1 {
                let p = Position(r, c)
                for i in 0..<4 {
                    guard self[p + BoxItAroundGame.offset2[i]][BoxItAroundGame.dirs[i]] != .line else {continue}
