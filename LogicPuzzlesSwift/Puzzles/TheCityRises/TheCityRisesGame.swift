...
class TheCityRisesGame: GridGame<TheCityRisesGameState> {
    ...
    init(layout: [String], delegate: TheCityRisesGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + TheCityRisesGame.offset2[i]][TheCityRisesGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + TheCityRisesGame.offset[i]]!)
                    }
                }
            }
        }
