...
class ZenLandscaperGameScene: GameScene<ZenLandscaperGameState> {
    ...
    func addThermometer(ch: Character, filled: Bool, s: AllowedObjectState, point: CGPoint, nodeName: String) {
        let n = ZenLandscaperGame.parts.getIndexOf(ch)!
    ...
    override func levelInitialized(_ game: AnyObject, state: ZenLandscaperGameState, skView: SKView) {
        ...
        addGrid(gridNode: ZenLandscaperGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols + 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows + 1) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: ZenLandscaperGameState, to stateTo: ZenLandscaperGameState) {
        ...
        func isFilled(o: ZenLandscaperObject) -> Bool {
