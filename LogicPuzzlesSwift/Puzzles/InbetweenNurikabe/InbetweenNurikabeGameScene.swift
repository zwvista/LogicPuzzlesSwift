//
//  InbetweenNurikabeGameScene.swift
//  LogicPuzzlesSwift
//
...
class InbetweenNurikabeGameScene: GameScene<InbetweenNurikabeGameState> {
    var gridNode: InbetweenNurikabeGridNode {
        get { getGridNode() as! InbetweenNurikabeGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: InbetweenNurikabeGameState, skView: SKView) {
        let game = game as! InbetweenNurikabeGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: InbetweenNurikabeGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
