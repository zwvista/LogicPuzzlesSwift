//
//  RippleEffectGameViewController.swift
//  LogicPuzzlesSwift
//
...
class RippleEffectGameViewController: GameGameViewController2<RippleEffectGameState, RippleEffectGame, RippleEffectDocument, RippleEffectGameScene> {
    ...
    override func viewDidLoad() {
        ...
        scene = RippleEffectGameScene(size: skView.bounds.size)
    ...
    override func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        var move = RippleEffectGameMove(p: p, obj: 0)
        if game.switchObject(move: &move) { soundManager.playSoundTap() }
    }
   
    override func newGame(level: GameLevel) -> RippleEffectGame {
        RippleEffectGame(layout: level.layout, delegate: self)
    }
}
