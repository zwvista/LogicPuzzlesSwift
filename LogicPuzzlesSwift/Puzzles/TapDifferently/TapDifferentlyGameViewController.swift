//
//  TapDifferentlyGameViewController.swift
//  LogicPuzzlesSwift
//
...
class TapDifferentlyGameViewController: GameGameViewController2<TapDifferentlyGameState, TapDifferentlyGame, TapDifferentlyDocument, TapDifferentlyGameScene> {
    ...
    override func viewDidLoad() {
        ...
        scene = TapDifferentlyGameScene(size: skView.bounds.size)
    ...
    override func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        var move = TapDifferentlyGameMove(p: p, obj: .empty)
        if game.switchObject(move: &move) { soundManager.playSoundTap() }
    }
    
    override func newGame(level: GameLevel) -> TapDifferentlyGame {
        TapDifferentlyGame(layout: level.layout, delegate: self)
    }
}
