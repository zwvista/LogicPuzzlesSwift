...
class TheOddBrickGame: GridGame<TheOddBrickGameViewController> {
    static let offset = [
        Position(-1, 0),
        Position(0, 1),
        Position(1, 0),
    ...
    init(layout: [String], delegate: TheOddBrickGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + TheOddBrickGame.offset2[i]][TheOddBrickGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + TheOddBrickGame.offset[i]]!)
                    }
                }
            }
        }
