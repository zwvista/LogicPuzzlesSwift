...
class TopArrowGame: GridGame<TopArrowGameState> {
    ...
    init(layout: [String], delegate: TopArrowGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + TopArrowGame.offset2[i]][TopArrowGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + TopArrowGame.offset[i]]!)
                    }
                }
            }
        }
        ...
        let state = TopArrowGameState(game: self)
