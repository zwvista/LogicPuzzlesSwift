//
//  ADifferentFarmerGameScene.swift
//  LogicPuzzlesSwift
//
...
class ADifferentFarmerGameScene: GameScene<ADifferentFarmerGameState> {
    var gridNode: ADifferentFarmerGridNode {
        get { getGridNode() as! ADifferentFarmerGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: ADifferentFarmerGameState, skView: SKView) {
        let game = game as! ADifferentFarmerGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: ADifferentFarmerGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: ADifferentFarmerGameState, to stateTo: ADifferentFarmerGameState) {
