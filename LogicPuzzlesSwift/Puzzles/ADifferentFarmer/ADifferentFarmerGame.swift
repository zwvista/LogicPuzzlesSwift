...
class ADifferentFarmerGame: GridGame<ADifferentFarmerGameState> {
    ...
    init(layout: [String], delegate: ADifferentFarmerGameViewController? = nil) {
        super.init(delegate: delegate)
        
        size = Position(layout.count / 2, layout[0].length / 2)
        dots = GridDots(rows: rows + 1, cols: cols + 1)
        objArray = Array<ADifferentFarmerObject>(repeating: .empty, count: rows * cols)
        ...
        for r in 0..<rows + 1 {
            ...
            for c in 0..<cols + 1 {
                ...
                let ch2 = str[2 * c + 1]
                self[r, c] = ADifferentFarmerObject(rawValue: ADifferentFarmerGame.chars.getIndexOf(ch2)!)!
            }
        }
        let g = Graph()
        ...
        let state = ADifferentFarmerGameState(game: self)
