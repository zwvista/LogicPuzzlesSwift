...
class CalcudokuGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: CalcudokuGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: CalcudokuGameState) {
        let game = game as! CalcudokuGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: CalcudokuGameState, to stateTo: CalcudokuGameState) {
        let game = game as! CalcudokuGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
