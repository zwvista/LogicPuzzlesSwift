...
class CalcudokuGameViewController: GameGameViewController {
    ...
    override func viewDidLoad() {
        ...
        scene = CalcudokuGameScene(size: skView.bounds.size)
    ...
    override func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        var move = CalcudokuGameMove(p: p, obj: 0)
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! CalcudokuGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! CalcudokuGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! CalcudokuGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! CalcudokuGameState, to: stateTo as! CalcudokuGameState)
