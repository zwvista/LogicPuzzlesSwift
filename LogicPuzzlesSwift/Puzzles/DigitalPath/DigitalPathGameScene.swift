...
class DigitalPathGameScene: GameScene<DigitalPathGameState> {
    var gridNode: DigitalPathGridNode {
        get { getGridNode() as! DigitalPathGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: DigitalPathGameState, skView: SKView) {
        let game = game as! DigitalPathGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // addGrid
        let offset:CGFloat = 0.5
        addGrid(gridNode: DigitalPathGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
