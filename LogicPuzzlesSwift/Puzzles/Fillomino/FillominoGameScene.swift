...
class FillominoGameScene: GameScene<FillominoGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: FillominoGameState, skView: SKView) {
        ...
        addGrid(gridNode: FillominoGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: FillominoGameState, to stateTo: FillominoGameState) {
        ...
        for r in 0..<stateFrom.rows {
            for c in 0..<stateFrom.cols {
                ...
                func f(state: FillominoGameState, p2: Position) -> Bool {
