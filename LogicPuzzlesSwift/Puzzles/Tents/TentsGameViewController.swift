...
class TentsGameViewController: GameViewController, GameDelegate, TentsMixin {
    ...
    func startGame() {
       ...
       let layout = gameDocument.levels[gameDocument.selectedLevelID]!
        ...
        game = TentsGame(layout: layout, delegate: self)
    ...
    func moveAdded(_ game: AnyObject, move: TentsGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func updateMovesUI(_ game: TentsGame) {
    ...
    func levelInitilized(_ game: AnyObject, state: TentsGameState) {
        let game = game as! TentsGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: TentsGameState, to stateTo: TentsGameState) {
        let game = game as! TentsGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
