...
class RomeGame: GridGame<RomeGameState> {
    ...
    init(layout: [String], delegate: RomeGameViewController? = nil) {
        super.init(delegate: delegate)
        
        size = Position(layout.count / 2, layout[0].length / 2)
        dots = GridDots(rows: rows + 1, cols: cols + 1)
        objArray = Array<RomeObject>(repeating: .empty, count: rows * cols)
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + RomeGame.offset2[i]][RomeGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + RomeGame.offset[i]]!)
                    }
                }
            }
        }
        ...
        let state = RomeGameState(game: self)
