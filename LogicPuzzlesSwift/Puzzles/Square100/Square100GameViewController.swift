...
class Square100GameViewController: GameGameViewController, GameDelegate {
    ...
    override func viewDidLoad() {
        ...
        scene = Square100GameScene(size: skView.bounds.size)
    ...
    func moveAdded(_ game: AnyObject, move: Square100GameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: Square100GameState) {
        let game = game as! Square100Game
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: Square100GameState, to stateTo: Square100GameState) {
        let game = game as! Square100Game
        updateMovesUI(game)
        guard !levelInitilizing else {return}
