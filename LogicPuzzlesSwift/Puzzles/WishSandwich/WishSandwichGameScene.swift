...
class WishSandwichGameScene: GameScene<WishSandwichGameState> {
    var gridNode: WishSandwichGridNode {
        get { getGridNode() as! WishSandwichGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: WishSandwichGameState, skView: SKView) {
        let game = game as! WishSandwichGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols + 1)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: WishSandwichGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols + 1) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows + 1) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: WishSandwichGameState, to stateTo: WishSandwichGameState) {
