...
class CastleBaileyGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: CastleBaileyGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: CastleBaileyGameState) {
        let game = game as! CastleBaileyGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: CastleBaileyGameState, to stateTo: CastleBaileyGameState) {
        let game = game as! CastleBaileyGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
