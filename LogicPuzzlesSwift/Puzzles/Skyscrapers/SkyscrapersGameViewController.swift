...
class SkyscrapersGameViewController: GameGameViewController {
    ...
    override func startGame() {
        ...
        game = SkyscrapersGame(layout: level.layout, delegate: self)
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! SkyscrapersGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! SkyscrapersGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! SkyscrapersGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! SkyscrapersGameState, to: stateTo as! SkyscrapersGameState)
