...
class SkyscrapersGameScene: GameScene<SkyscrapersGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: SkyscrapersGameState, skView: SKView) {
        ...
        addGrid(gridNode: SkyscrapersGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
