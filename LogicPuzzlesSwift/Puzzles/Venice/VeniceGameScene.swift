...
class VeniceGameScene: GameScene<VeniceGameState> {
    var gridNode: VeniceGridNode {
        get { getGridNode() as! VeniceGridNode }
    ...
    override func levelInitialized(_ game: AnyObject, state: VeniceGameState, skView: SKView) {
        let game = game as! VeniceGame
        removeAllChildren()
        let blockSize = CGFloat(skView.bounds.size.width) / CGFloat(game.cols)
        
        // add Grid
        let offset:CGFloat = 0.5
        addGrid(gridNode: VeniceGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
