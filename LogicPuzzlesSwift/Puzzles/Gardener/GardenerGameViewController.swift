...
class GardenerGameViewController: GameGameViewController, GameDelegate {
    ...
    func moveAdded(_ game: AnyObject, move: GardenerGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: GardenerGameState) {
        let game = game as! GardenerGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: GardenerGameState, to stateTo: GardenerGameState) {
        let game = game as! GardenerGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
