...
class LitsGameScene: GameScene<LitsGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: LitsGameState, skView: SKView) {
        ...
        addGrid(gridNode: LitsGridNode(blockSize: blockSize, rows: game.rows, cols: game.cols), point: CGPoint(x: skView.frame.midX - blockSize * CGFloat(game.cols) / 2 - offset, y: skView.frame.midY + blockSize * CGFloat(game.rows) / 2 + offset))
    ...
    override func levelUpdated(from stateFrom: LitsGameState, to stateTo: LitsGameState) {
