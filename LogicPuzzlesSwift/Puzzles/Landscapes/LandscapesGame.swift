...
class LandscapesGame: GridGame<LandscapesGameState> {
    ...
    init(layout: [String], sum: Int, delegate: LandscapesGameViewController? = nil) {
        ...
        for r in 0..<rows {
            for c in 0..<cols {
                let p = Position(r, c)
                for i in 0..<4 {
                    if dots[p + LandscapesGame.offset2[i]][LandscapesGame.dirs[i]] != .line {
                        g.addEdge(pos2node[p]!, neighbor: pos2node[p + LandscapesGame.offset[i]]!)
                    }
                }
            }
        }
        ...
        let state = LandscapesGameState(game: self)
