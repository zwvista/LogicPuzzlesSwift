...
class MasyuGameScene: GameScene<MasyuGameState> {
    ...
    override func levelInitialized(_ game: AnyObject, state: MasyuGameState, skView: SKView) {
        ...
        gridNode = MasyuGridNode(blockSize: blockSize, rows: game.rows - 1, cols: game.cols - 1)
    ...
    override func levelUpdated(from stateFrom: MasyuGameState, to stateTo: MasyuGameState) {
        ...
        for row in 0..<stateFrom.rows {
            for col in 0..<stateFrom.cols {
                ...
                func addHorzLine(objType: MasyuObject) {
                ...
                func removeHorzLine(objType: MasyuObject) {
                    if objType != .empty { removeNode(withName: horzLineNodeName) }
                }
                func addVertLine(objType: MasyuObject) {
                    guard objType != .empty else {return}
                    let pathToDraw = CGMutablePath()
                    let lineNode = SKShapeNode(path:pathToDraw)
                    switch objType {
                ...
                func removeVertLine(objType: MasyuObject) {
