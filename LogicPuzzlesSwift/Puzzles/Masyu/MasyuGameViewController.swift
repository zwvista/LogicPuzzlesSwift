...
class MasyuGameViewController: GameGameViewController2<MasyuGameState, MasyuGame, MasyuDocument, MasyuGameScene> {
    ...
    override func moveAdded(_ game: AnyObject, move: Any) {
        ...
        gameDocument.moveAdded(game: game, move: move as! MasyuGameMove)
    }
    
    override func levelInitilized(_ game: AnyObject, state: AnyObject) {
        let game = game as! MasyuGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state as! MasyuGameState, skView: skView)
    }
    ...
    override func levelUpdated(_ game: AnyObject, from stateFrom: AnyObject, to stateTo: AnyObject) {
        ...
        updateMovesUI(game)
        guard !levelInitilizing else {return}
        scene.levelUpdated(from: stateFrom as! MasyuGameState, to: stateTo as! MasyuGameState)
