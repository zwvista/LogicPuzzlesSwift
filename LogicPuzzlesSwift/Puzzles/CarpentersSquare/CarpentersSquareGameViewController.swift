...
class CarpentersSquareGameViewController: GameGameViewController, GameDelegate {
    ...
    var game: CarpentersSquareGame {
        get {return getGame() as! CarpentersSquareGame}
    ...
    func moveAdded(_ game: AnyObject, move: CarpentersSquareGameMove) {
        guard !levelInitilizing else {return}
        gameDocument.moveAdded(game: game, move: move)
    }
    
    func levelInitilized(_ game: AnyObject, state: CarpentersSquareGameState) {
        let game = game as! CarpentersSquareGame
        updateMovesUI(game)
        scene.levelInitialized(game, state: state, skView: skView)
    }
    
    func levelUpdated(_ game: AnyObject, from stateFrom: CarpentersSquareGameState, to stateTo: CarpentersSquareGameState) {
        let game = game as! CarpentersSquareGame
        updateMovesUI(game)
        guard !levelInitilizing else {return}
